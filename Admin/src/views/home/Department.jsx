...
function Department() {
  ...
  async function fetchDeparment() {
    try {
      ...
      const response = await axios.get(baseUrl + "/admin/get-department");
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        // Send a request to the server to authenticate the user
        const response = await axios.post(baseUrl + "/admin/create-department", {
      ...
      <form className="flex items-center max-lg:justify-center gap-0">
        <div className="relative">
          <input
            type="search"
            id="default-search"
            className="block w-full max-w-xs px-4 py-2 text-sm font-normal shadow-xs text-gray-900 bg-white border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none leading-relaxed"
            placeholder="Search... "
            required=""
          />
        </div>
        ...
        <table className="w-full rounded-xl">
          ...
          <tbody className="divide-y divide-gray-300">
            {departments.map((department, index) => (
              <tr key={department._id} className="bg-white transition-all duration-500 hover:bg-gray-50">
      ...
      <div className="transform -translate-y-3 ease-out sm:max-w-lg sm:w-full m-5 sm:mx-auto transition-all modal-open:translate-y-0 modal-open:opacity-100 modal-open:duration-500">
        <div className="flex flex-col bg-white rounded-2xl py-4 px-5">
          <div className="flex justify-between items-center pb-4 border-b border-gray-200">
            <h4 className="text-sm text-gray-900 font-medium">Add Department</h4>
            <button
